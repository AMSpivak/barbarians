...
layout (location = 3) in ivec4 indexOfBone;
...
void main()
{
	...
	tv_Position = model *(vec4(tv_Position.xyz, 1.0));
		...
		FragPosLightSpace = lightSpaceMatrix * tv_Position;//vec4(v_Position,1.0);
		
        gl_Position = camera * tv_Position;//vec4(v_Position,1.0);


    	InvTexCoord.x = texCoord.x;
		InvTexCoord.y = 1.0 - texCoord.y;
}
